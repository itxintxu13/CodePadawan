...
// Endpoint GET para obtener código Python por ID
export async function GET(req: NextRequest) {
  ...
  const snap = await get(ref(database, `blogs/python/codes/${id}`));
  
  // Retorna el código o una cadena vacía si no existe
  return NextResponse.json({ code: snap.val()?.code || "" });
}

// Endpoint POST para guardar nuevo código Python
export async function POST(req: NextRequest) {
  ...
  const newRef = push(ref(database, "blogs/python/codes"));
