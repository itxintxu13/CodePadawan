...
const filePath = path.join(process.cwd(), "src/app/blog/javascript/comments.json");
...
if (!fs.existsSync(filePath) || fs.statSync(filePath).size === 0) {
  console.warn("⚠️ El archivo JSON está vacío, inicializándolo...");
...
export async function GET() {
  try {
    ...
    if (!data.trim()) {
      throw new Error("El archivo JSON está vacío o malformado.");
    }
