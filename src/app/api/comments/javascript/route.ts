...
export async function GET() {
  try {
    ...
    const comments = baseUser?.publicMetadata?.commentsJavascript || [];
...
export async function POST(req: NextRequest) {
  try {
    ...
    const comments = (baseUser?.publicMetadata?.commentsJavascript || []) as any[];
    ...
    for (const u of allUsers) {
      await clerkClient.users.updateUser(u.id, {
        publicMetadata: {
          ...
          commentsJavascript: updatedComments 
        }
      });
    }

    return NextResponse.json({ commentsJavaScript: updatedComments[updatedComments.length - 1] });
  } catch (error) {
    ...
    for (const u of allUsers) {
      await clerkClient.users.updateUser(u.id, {
        publicMetadata: {
          points: u.publicMetadata?.points ?? 0,
          retosResueltos: u.publicMetadata?.retosResueltos ?? 0,
          commentsJavascript: [] // 👈 SOLO este campo
        }
      });
    }
